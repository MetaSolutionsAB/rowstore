...
public class ConfigurableTestEnvironment {
    ...
    private void createConfig() {
        try {
            ...
            // Note: RowStoreConfig expects lowercase keys (timerange, global, dataset, clientip)
    ...
    private void startRowStore() {
        try {
            ...
            Server server = restletComponent.getServers().add(Protocol.HTTP, serverPort);

            restletComponent.getLogService().setResponseLogFormat(
                    "{ciua} \"{m} {rp} {rq}\" {S} {ES} {es} {hh} {cig} {fi}");
            server.getContext().getParameters().add("useForwardedForHeader", "true");
            restletComponent.getClients().add(Protocol.HTTP);
            restletComponent.getClients().add(Protocol.HTTPS);
